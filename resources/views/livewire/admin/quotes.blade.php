    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-3">
        <div class="flex items-center gap-2 flex-1">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar folio o cliente..."
                class="bg-white border border-gray-200 rounded-lg px-4 py-2 text-sm flex-1 focus:outline-none focus:border-orange-400">
        ...
        <a href="{{ route('admin.quotes.create') }}"
            class="bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors text-center">
            + Nueva cotización
        </a>
    </div>
    ...
    <div class="lg:hidden space-y-3">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            <div class="flex items-start justify-between mb-2">
                <div>
                    <p class="font-semibold text-gray-800">{{ $quote->folio }}</p>
                ...
                <div class="text-right">
                    <p class="font-bold text-gray-900">${{ number_format($quote->total, 2) }}</p>
                    <p class="text-xs text-gray-400">{{ $quote->created_at->format('d/m/Y') }}</p>
                </div>
            </div>
            <div class="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('admin.quotes.show', $quote) }}"
                        class="text-orange-500 text-sm font-medium">Ver detalle</a>
        ...
        @empty
        <p class="text-center text-gray-400 py-8">No hay cotizaciones.</p>
        @endforelse
    </div>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-3 text-right space-x-2">
                        <a href="{{ route('admin.quotes.show', $quote) }}" class="text-gray-400 hover:text-orange-500 transition-colors">
                            <svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        </a>
                        <button wire:click="delete({{ $quote->id }})" wire:confirm="¿Eliminar esta cotización?" class="text-gray-400 hover:text-red-500 transition-colors">
                            <svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                        </button>
                    </td>
                </tr>
                @empty
                <tr><td colspan="6" class="px-6 py-8 text-center text-gray-400">No hay cotizaciones.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
